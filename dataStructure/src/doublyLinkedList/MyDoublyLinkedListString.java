 ...
 * @date 2018.08.28
 */
public class MyDoublyLinkedListString {
	 ...
	 * @author kimgun
	 */
	public MyDoublyLinkedListString() {
		first = null;
		size = 0;
	}

	/**
	 * (노드 객체) 데이터와 다음 객체의 주소를 가짐
	 * 
	 * @author kimgun
	 */
	private class Node {
	...
	public boolean add(String str) {
		try {
			...
			if (first == null) {
				...
				Node beforeNode = searchIndexNode(size - 1);
				beforeNode.next = newNode;
				newNode.pre = beforeNode;
				last = beforeNode.next;
			}
		} catch (Exception e) {
			return false;
		}
